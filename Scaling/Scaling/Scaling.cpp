...
int main(int argc, char** argv)
{
    ...
    Mat raw_image(image.rows/3, image.cols/3, CV_8UC3);
    int index = 0;
    ...
    for (int i = 0; i < image.rows; i += 3) {
        for (int j = 0; j < image.cols; j += 3) {
            ...
            redMatrix[rowCounter][coloumnCounter] = pixel[2];
            greenMatrix[rowCounter][coloumnCounter] = pixel[1];
            blueMatrix[rowCounter][coloumnCounter++] = pixel[0];
        }
        coloumnCounter = 0;
        rowCounter++;
    }
    ...
    waitKey(0);
    imwrite("Resources/behruz_oneThirdScale.jpg", raw_image);
